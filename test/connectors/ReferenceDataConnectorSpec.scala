  ...
  "Reference Data" - {
    ...
    "getCustomsOfficesOfTransitForCountry" - {
      "must return a successful future response with a sequence of CustomsOffices" in {
        server.stubFor(
          get(urlEqualTo(s"/$baseUrl/customs-offices-p5/GB?role=TRA"))
      ...
      "must return a successful future response when CustomsOffice is not found" in {
        server.stubFor(
          get(urlEqualTo(s"/$baseUrl/customs-offices-p5/AR?role=TRA")).willReturn(
            aResponse()
      ...
      "must return an exception when an error response is returned" in {
        checkErrorResponse(s"/$baseUrl/customs-offices-p5/GB?role=TRA", connector.getCustomsOfficesOfTransitForCountry(CountryCode("GB")))
      }
    }
    ...
    "getCustomsOfficesOfDestinationForCountry" - {
      "must return a successful future response with a sequence of CustomsOffices" in {
        server.stubFor(
          get(urlEqualTo(s"/$baseUrl/customs-offices-p5/GB?role=DES"))
            .willReturn(okJson(customsOfficeDestinationResponseJson))
        )
      ...
      "must return a successful future response when CustomsOffice is not found" in {
        server.stubFor(
          get(urlEqualTo(s"/$baseUrl/customs-offices-p5/AR?role=DES")).willReturn(
            aResponse()
              .withStatus(NOT_FOUND)
              .withHeader(CONTENT_TYPE, JSON)
          )
      ...
      "must return an exception when an error response is returned" in {
        checkErrorResponse(s"/$baseUrl/customs-offices-p5/GB?role=DES", connector.getCustomsOfficesOfDestinationForCountry(CountryCode("GB")))
      }
    }
