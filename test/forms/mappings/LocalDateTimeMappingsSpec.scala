...
class LocalDateTimeMappingsSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with Generators with OptionValues with Mappings {

  val form = Form(
    "value" -> localDateTime(
      requiredKey = "error.required",
      allRequiredKey = "error.required.all",
      multipleRequiredKey = "error.required.multiple",
  ...
  val validData = dateTimesBetween(
    ...
    max = LocalDateTime.of(3000, 1, 1, 0, 0, 0)
  )
  ...
  "must bind valid data" in {
    forAll(validData -> "valid date") {
      date =>
        ...
        val dateToCheck = date.minusSeconds(date.getSecond).minusNanos(date.getNano)
        result.value.value mustEqual dateToCheck
    }
  }
  ...
  "must fail to bind a date with a missing hour" in {
  ...
  "must fail to bind a date with an invalid hour" in {
  ...
  "must fail to bind a date with a missing minute" in {
    ...
    forAll(validData -> "valid date", missingField -> "missing field") {
      (date, field) =>
        ...
        val data = field.fold(initialData) {
          value =>
            initialData + ("value.minute" -> value)
        }
  ...
  "must fail to bind a date with an invalid minute" in {
    ...
    forAll(validData -> "valid date", invalidField -> "invalid field") {
      (date, field) =>
        val data = Map(
          ...
          "value.month"  -> date.getMonthValue.toString,
          "value.year"   -> date.getYear.toString,
          "value.hour"   -> date.getMinute.toString,
          "value.minute" -> field
        )
  ...
  "must fail to bind a date with a missing day and year" in {
    ...
    forAll(validData -> "valid date", missingField -> "missing day", missingField -> "missing year") {
      (date, dayOpt, yearOpt) =>
        ...
        result.errors must contain only FormError("value", "error.required.multiple", List("year", "day"))
    }
  }
  ...
  "must fail to bind a date with a missing month and year" in {
    ...
    forAll(validData -> "valid date", missingField -> "missing month", missingField -> "missing year") {
      (date, monthOpt, yearOpt) =>
        ...
        result.errors must contain only FormError("value", "error.required.multiple", List("year", "month"))
    }
  }

  "must fail to bind a date with a missing hour and minute" in {

    forAll(validData -> "valid date", missingField -> "missing hour", missingField -> "missing minute") {
      (date, hourOpt, minuteOpt) =>
        val hour = hourOpt.fold(Map.empty[String, String]) {
          value =>
            Map(
              "value.hour" -> value
            )
        ...
        val minute = minuteOpt.fold(Map.empty[String, String]) {
          value =>
        ...
        val data: Map[String, String] = Map(
          "value.day"   -> date.getDayOfMonth.toString,
          "value.month" -> date.getMonthValue.toString,
          "value.year"  -> date.getDayOfYear.toString
        ) ++ minute ++ hour

        val result = form.bind(data)

        result.errors must contain only FormError("value", "error.required.multiple", List("minute", "hour"))
    }
  }

  "must fail to bind a date with a missing day, month and year" in {

    forAll(validData -> "valid date", missingField -> "missing day", missingField -> "missing month", missingField -> "missing year") {
      (date, dayOpt, monthOpt, yearOpt) =>
        val day = dayOpt.fold(Map.empty[String, String]) {
          value =>
            Map("value.day" -> value)
        }
        ...
        result.errors must contain only FormError("value", "error.required.multiple", List("year", "day", "month"))
    }
  }
  ...
  "must unbind a date" in {
    ...
    forAll(validData -> "valid date") {
      date =>
        ...
        filledForm("value.hour").value.value mustEqual date.getHour.toString
        filledForm("value.minute").value.value mustEqual date.getMinute.toString
    }
  }
}
