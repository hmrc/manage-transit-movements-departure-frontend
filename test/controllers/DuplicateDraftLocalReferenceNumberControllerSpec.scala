  ...
  "LocalReferenceNumber Controller" - {
    ...
    "must create new user answers" - {
      "with old LRN data" in {
        ...
        when(mockDuplicateService.copyUserAnswers(eqTo(lrn), eqTo(newUserAnswers.lrn))(any())).thenReturn(Future.successful(true)) // TODO notSubmitted
        ...
        verify(mockDuplicateService).copyUserAnswers(eqTo(lrn), eqTo(newUserAnswers.lrn))(any())
    ...
    "must redirect to technicalDifficulties" - {
      "when get users answers returns None" in {
        ...
        when(mockDuplicateService.doesIE028ExistForLrn(any())(any())).thenReturn(Future.successful(alreadyExists))
        when(mockDuplicateService.alreadySubmitted(any())(any())).thenReturn(Future.successful(alreadyExists))
        when(mockDuplicateService.copyUserAnswers(any(), any())(any())).thenReturn(Future.successful(true))
      ...
      "when delete returns false" in {
        ...
        val alreadyExists: Boolean = false
        when(mockDuplicateService.alreadySubmitted(eqTo(Some(newUserAnswers.lrn)))(any())).thenReturn(Future.successful(alreadyExists))
        when(mockDuplicateService.copyUserAnswers(eqTo(lrn), eqTo(newUserAnswers.lrn))(any())).thenReturn(Future.successful(true))
        when(mockSessionRepository.get(eqTo(newUserAnswers.lrn))(any())) thenReturn Future.successful(Some(newUserAnswers))
