...
class ApprovedOperatorControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {

  private val formProvider               = new YesNoFormProvider()
  private val form                       = formProvider("traderDetails.consignment.approvedOperator")
  private val mode                       = NormalMode
  private lazy val approvedOperatorRoute = routes.ApprovedOperatorController.onPageLoad(lrn, mode).url
  ...
  "ApprovedOperator Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val view = injector.instanceOf[ApprovedOperatorView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =emptyUserAnswers.set(ApprovedOperatorPage, true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, approvedOperatorRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request   = FakeRequest(POST, approvedOperatorRoute).withFormUrlEncodedBody(("value", ""))
