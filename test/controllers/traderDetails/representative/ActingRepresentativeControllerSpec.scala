...
class ActingRepresentativeControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {

  private val formProvider                   = new YesNoFormProvider()
  private val form                           = formProvider("traderDetails.representative.actingRepresentative")
  private val mode                           = NormalMode
  private lazy val actingRepresentativeRoute = routes.ActingRepresentativeController.onPageLoad(lrn, mode).url
  ...
  "ActingRepresentative Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val view = injector.instanceOf[ActingRepresentativeView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(lrn, eoriNumber).set(ActingRepresentativePage, true).success.value
