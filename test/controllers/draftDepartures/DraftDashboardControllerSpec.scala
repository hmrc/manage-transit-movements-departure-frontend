package controllers.draftDepartures

import base.{AppWithDefaultMockFixtures, SpecBase}
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.draftDepartures.DraftDashboardView

class DraftDashboardControllerSpec extends SpecBase with AppWithDefaultMockFixtures {

  private lazy val draftDashboardRoute = routes.DraftDashboardController.onPageLoad(lrn).url

  "DraftDashboard Controller" - {

    "must return OK and the correct view for a GET" in {

      setExistingUserAnswers(emptyUserAnswers)

      val request = FakeRequest(GET, draftDashboardRoute)
      val result  = route(app, request).value

      val view = injector.instanceOf[DraftDashboardView]

      status(result) mustEqual OK

      contentAsString(result) mustEqual
        view(lrn)(request, messages).toString
    }
  }
}
