...
class CustomsOfficeActiveBorderControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private val customsOffice1    = arbitrary[CustomsOffice].sample.value
  ...
  "ActiveBorderOfficeTransit Controller" - {
    ...
    "must return OK and the correct view for a GET" - {
      "when destination office defined at index" in {
        val destinationOffice = arbitrary[CustomsOffice].sample.value

        val updatedAnswers = emptyUserAnswers
          .setValue(OfficeOfDeparturePage, destinationOffice)
        ...
        contentAsString(result) mustEqual
          view(form, lrn, customsOfficeList, mode, index)(request, messages).toString
