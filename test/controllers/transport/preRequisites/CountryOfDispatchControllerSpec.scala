...
class CountryOfDispatchControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private val form         = formProvider("transport.preRequisites.countryOfDispatch", countryList)
  private val mode         = NormalMode

  private val mockCountriesService: CountriesService = mock[CountriesService]
  private lazy val countryOfDispatchRoute            = routes.CountryOfDispatchController.onPageLoad(lrn, mode).url
  ...
  "CountryOfDispatch Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      when(mockCountriesService.getCountries()(any())).thenReturn(Future.successful(countryList))
      val userAnswers = emptyUserAnswers.setValue(CountryOfDispatchPage, country1)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request = FakeRequest(POST, countryOfDispatchRoute)
