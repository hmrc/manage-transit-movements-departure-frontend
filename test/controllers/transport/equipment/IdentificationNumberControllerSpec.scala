...
class IdentificationNumberControllerSpec extends SpecBase with AppWithDefaultMockFixtures {
  ...
  private def form(otherIds: Seq[String] = Nil)       = formProvider("transport.equipment.identificationNumber", otherIds)
  private val validAnswer                             = "testString"
  private val mode                                    = NormalMode
  private def identificationNumberRoute(index: Index) = routes.IdentificationNumberController.onPageLoad(lrn, mode, index).url
  ...
  "IdentificationNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(IdentificationNumberPage(equipmentIndex), validAnswer)
