...
import pages.transport.equipment.index.seals.ContainerSealIdentificationNumberPage
import play.api.inject.bind
import play.api.inject.guice.GuiceApplicationBuilder
import play.api.test.FakeRequest
import play.api.test.Helpers._
import views.html.transport.equipment.index.seals.ContainerSealIdentificationNumberView
...
class ContainerSealIdentificationNumberControllerSpec extends SpecBase with AppWithDefaultMockFixtures {
  ...
  private val form                                        = formProvider("transport.equipment.index.seals.containerSealIdentificationNumber")
  private val mode                                        = NormalMode
  private lazy val containerSealIdentificationNumberRoute = routes.ContainerSealIdentificationNumberController.onPageLoad(lrn, mode).url
  ...
  "ContainerSealIdentificationNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(ContainerSealIdentificationNumberPage, "test string")
