...
import views.html.routeDetails.BindingItineraryView
...
class BindingItineraryControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {
  ...
  "BindingItinerary Controller" - {

    "must return OK and the correct view for a GET" in {

      setExistingUserAnswers(emptyUserAnswers)

      val request = FakeRequest(GET, bindingItineraryRoute)
      val result  = route(app, request).value

      val view = injector.instanceOf[BindingItineraryView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.setValue(BindingItineraryPage, true)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request = FakeRequest(POST, bindingItineraryRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request   = FakeRequest(POST, bindingItineraryRoute).withFormUrlEncodedBody(("value", ""))
