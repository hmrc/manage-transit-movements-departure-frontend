...
import views.html.routeDetails.loading.PlaceOfLoadingCountryView

class PlaceOfLoadingCountryViewSpec extends InputSelectViewBehaviours[Country] with Generators {
  ...
  override def applyView(form: Form[Country]): HtmlFormat.Appendable =
    injector.instanceOf[PlaceOfLoadingCountryView].apply(form, lrn, values, NormalMode)(fakeRequest, messages)

  override val prefix: String = "routeDetails.loading.placeOfLoadingCountry"
