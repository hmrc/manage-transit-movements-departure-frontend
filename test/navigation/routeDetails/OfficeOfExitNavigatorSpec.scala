...
class OfficeOfExitNavigatorSpec extends SpecBase with ScalaCheckPropertyChecks with Generators with RouteDetailsUserAnswersGenerator {
  ...
  private val navigator = new OfficeOfExitNavigator(index, ctcCountryCodes, euCountryCodes, customsSecurityAgreementAreaCountryCodes)
          ...
          forAll(arbitraryOfficeOfExitAnswers(emptyUserAnswers, index)) {
            answers =>
              navigator
                ...
                .mustBe(controllers.routeDetails.officeOfExit.routes.CheckOfficeOfExitAnswersController.onPageLoad(lrn, index))
          }
        }
      }
    }
