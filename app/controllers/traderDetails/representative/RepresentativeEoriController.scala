...
class RepresentativeEoriController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(RepresentativeEoriPage) match {
        case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(RepresentativeEoriPage, value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(RepresentativeEoriPage, mode, updatedAnswers))
        )
  }
}
