...
import views.html.routeDetails.transit.ConfirmRemoveOfficeOfTransitView
          ...
          formWithErrors => Future.successful(BadRequest(view(formWithErrors, lrn, index))),
          {
            case true =>
              OfficeOfTransitCountrySection(index)
                ...
                .writeToSession()
                .navigateTo(routes.AddAnotherOfficeOfTransitController.onPageLoad(lrn))
            case false =>
              Future.successful(Redirect(routes.AddAnotherOfficeOfTransitController.onPageLoad(lrn)))
          }
        )
  }
}
