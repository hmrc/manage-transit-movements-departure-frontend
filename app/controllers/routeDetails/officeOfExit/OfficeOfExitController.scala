...
class OfficeOfExitController @Inject() (
    ...
    .async {
      implicit request =>
        ...
        service.getCustomsOfficesOfExitForCountry(country.code).map {
          customsOfficeList =>
            val form = formProvider("routeDetails.officeOfExit.officeOfExit", customsOfficeList, country.description)
            val preparedForm = request.userAnswers.get(officeOfExit.index.OfficeOfExitPage(index)) match {
              case None        => form
  ...
  def onSubmit(lrn: LocalReferenceNumber, index: Index, mode: Mode): Action[AnyContent] = actions
    .requireData(lrn)
    .andThen(getMandatoryPage(officeOfExit.index.OfficeOfExitCountryPage(index)))
    .async {
      implicit request =>
        val country = request.arg
          ...
          customsOfficeList =>
            ...
            form
              ...
              .fold(
                ...
                value => officeOfExit.index.OfficeOfExitPage(index).writeToUserAnswers(value).writeToSession().navigateWith(mode)
              )
        }
    }
}
