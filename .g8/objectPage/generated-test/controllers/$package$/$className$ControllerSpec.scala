...
class $className$ControllerSpec extends SpecBase with AppWithDefaultMockFixtures with Generators {
  ...
  private val validValue                  = arbitrary$objectClassName$.arbitrary.sample.get
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(lrn, eoriNumber).set($className$Page, validValue).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, $className;format="decap"$Route)
          .withFormUrlEncodedBody(("value", validValue.value))
